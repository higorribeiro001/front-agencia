...
import { plate, plateFindByName, plates, postPlate, putPlate } from "../service/api/plate";
...
export default function Plate() {
    ...
    const [dataSeller, setDataPlateDrawer] = useState<DataInterface>();
    ...
    const columnsPlate: GridColDef[] = [
        ...
        { field: 'acao', headerName: 'Ação', flex: 1 , renderCell: (data) => (
            <IconButton onClick={() => getPlateDrawer(String(data.id))}>
              <Visibility />
            </IconButton>
        ), },
    ];
    ...
    const handleSearch = (e: ChangeEvent<HTMLInputElement>) => {
        ...
        timeout = setTimeout(async () => {
            if (e.target.value !== "") {
                const dataDrawer = await plateFindByName(e.target.value);
    ...
    const ContentViewPlate = ({dataData}: DataDataInterface) => {
        return (
            <div className="flex flex-col gap-1">
                <h2 className="text-primary text-[25px] font-semibold mb-2">PLACA</h2>
                <RowDrawer
                    keyRow="Nome"
                    value={dataData?.nome ?? ''}
                />
    ...
    const getDrawer = async () => {
        const dataPlate = await plates(currentPage);
        const categoriesData = new DataAdapts(dataPlate!);

        const dataPlateAdapt = categoriesData.externalDataAdapts;
        setRowsDrawer(dataPlateAdapt?.data);
        const numPages = dataPlateAdapt?.last_page;
        setPages(numPages);
        setIsLoading(false);
    }
    ...
    const submitRegisterPlate = async (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
    ...
    const registerPlate = async () => {
        try {
            const response = await postPlate(
                { 
                    nome: model.value
                }
            );
    ...
    const submitEditPlate = async (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
    ...
    return (
        ...
        >
            ...
            <DialogRegister 
                ...
                title="Cadastro de Placa"
                valueInput={model.value}
                errorInput={model.error}
                funcInput={changeValues}
                funcRegister={submitRegisterPlate}
                handleClose={handleCloseRegister}
                isLoading={isLoadingRegister}
            />
            <DialogRegister 
                isOpen={openDialogEdit}
                title="Edição de Placa"
                valueInput={model.value}
                errorInput={model.error}
                funcInput={changeValues}
                funcRegister={submitEditPlate}
