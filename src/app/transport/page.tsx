...
import { numTransport, numTransportFindByName, numTransports, postNumTransport, putNumTransport } from "../service/api/numTransport";
...
export default function Transport() {
    ...
    const [dataSeller, setDataTransportDrawer] = useState<DataInterface>();
    ...
    const getTransportDrawer = async (id: string) => {
        const dataTransport = await numTransport(id);
        const transportAdapt = new DataAdapt(dataTransport);

        setOpenDrawer(true);
        setDataTransportDrawer(transportAdapt.externalDataAdapt)
    }
    ...
    const columnsTransport: GridColDef[] = [
        ...
        { field: 'acao', headerName: 'Ação', flex: 1 , renderCell: (data) => (
            <IconButton onClick={() => getTransportDrawer(String(data.id))}>
              <Visibility />
            </IconButton>
        ), },
    ];
    ...
    const handleSearch = (e: ChangeEvent<HTMLInputElement>) => {
        ...
        timeout = setTimeout(async () => {
            if (e.target.value !== "") {
                const dataDrawer = await numTransportFindByName(e.target.value);
    ...
    const ContentViewTransport = ({dataData}: DataDataInterface) => {
        return (
            <div className="flex flex-col gap-1">
                <h2 className="text-primary text-[25px] font-semibold mb-2">Transporte</h2>
                <RowDrawer
                    keyRow="Nome"
                    value={dataData?.nome ?? ''}
                />
    ...
    const getDrawer = async () => {
        const dataTransports = await numTransports(currentPage);
        const categoriesData = new DataAdapts(dataTransports!);

        const dataTransportsAdapt = categoriesData.externalDataAdapts;
        setRowsDrawer(dataTransportsAdapt?.data);
        const numPages = dataTransportsAdapt?.last_page;
        setPages(numPages);
        setIsLoading(false);
    }
    ...
    const handleOpenEdit = (id: string) => {
        ...
        setIdName(id);
        const getName = async () => {
            const name = await numTransport(id);
            const transportAdapt = new DataAdapt(name);
            setModel((prevModel) => ({
                ...prevModel, 
                value: transportAdapt.externalDataAdapt.nome
            }));
        }
    ...
    const submitRegisterTransport = async (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
    ...
    const registerTransport = async () => {
        try {
            const response = await postNumTransport(
                { 
                    nome: model.value
                }
            );
    ...
    const submitEditTransport = async (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
    ...
    const editTransport = async () => {
        try {
            const response = await putNumTransport(
            { 
                id: idName,
                nome: model.value
            });
    ...
    return (
        ...
        >
            ...
            <DialogRegister 
                ...
                title="Cadastro de Transporte"
                valueInput={model.value}
                errorInput={model.error}
                funcInput={changeValues}
                funcRegister={submitRegisterTransport}
                handleClose={handleCloseRegister}
                isLoading={isLoadingRegister}
            />
            <DialogRegister 
                isOpen={openDialogEdit}
                title="Edição de Transporte"
                valueInput={model.value}
                errorInput={model.error}
                funcInput={changeValues}
                funcRegister={submitEditTransport}
