...
import { client, clientFindByName, clients, postClient, putClient } from "../service/api/client";
...
export default function Client() {
    ...
    const [dataSeller, setDataClientDrawer] = useState<SellerInterface>();
    ...
    const columnsClient: GridColDef[] = [
        ...
        { field: 'acao', headerName: 'Ação', flex: 1 , renderCell: (data) => (
            <IconButton onClick={() => getClientDrawer(String(data.id))}>
              <Visibility />
            </IconButton>
        ), },
    ];
    ...
    const handleSearch = (e: ChangeEvent<HTMLInputElement>) => {
        ...
        timeout = setTimeout(async () => {
            if (e.target.value !== "") {
                const dataDrawer = await clientFindByName(e.target.value);
    ...
    const ContentViewClient = ({dataData}: DataDataInterface) => {
        return (
            <div className="flex flex-col gap-1">
                <h2 className="text-primary text-[25px] font-semibold mb-2">CLIENTE</h2>
                <RowDrawer
                    keyRow="Nome"
                    value={dataData?.nome ?? ''}
                />
    ...
    const getDrawer = async () => {
        const dataClients = await clients(currentPage);
        const clientsData = new DataAdapts(dataClients!);

        const dataClientsAdapt = clientsData.externalDataAdapts;
        setRowsDrawer(dataClientsAdapt?.data);
        const numPages = dataClientsAdapt?.last_page;
        setPages(numPages);
        setIsLoading(false);
    }
    ...
    const submitRegisterClient = async (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
    ...
    const submitEditClient = async (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
    ...
    return (
        ...
        >
            ...
            <DialogRegister 
                ...
                funcRegister={submitRegisterClient}
