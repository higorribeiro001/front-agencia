...
export default function EditChartAccount({ params }: { params: Promise<{ id: string }> }) {
    ...
    useEffect(() => {
      ...
      const getChartAccount = async () => {
        const dataChartAccount: ChartAccountInterface | undefined = await chartAccount(resolvedParams.id);
        const chartAccountAdapt = new ChartAccountAdapt(dataChartAccount!);

        const chartAccountData = chartAccountAdapt.externalChartAccountAdapt;
    ...
    return (
        <Base 
          title="Edição de Plano de Contas"
          openAlert={openAlert}
          isSuccess={isSuccess}
