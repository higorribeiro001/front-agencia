...
import { postEmployee } from "@/app/service/api/employees";
...
export default function RegisterEmployee() {
    ...
    const submitForm = async (e: React.FormEvent<HTMLFormElement>) => {
      ...
      const registerMdfDoorLeave = async () => {
        try {
          const response = await postEmployee(parseInt(model[0].value), model[1].value, model[2].value, model[3].value, model[4].value, model[5].value, model[6].value);
                        ...
                        {formFields.map((value, index: number) => (
                            value.type === 'select' ? (
                              <Autocomplete
                                  key={index}
                                  disablePortal
                                  options={value.name === 'mes' ? meses : value.name === 'setor' ? setores : value.name === 'funcao' ? funcoes : value.name === 'categoria' ? categorias : categorias_bonus}
                                  sx={{ width: 300 }}
                                  className="w-[49%]"
