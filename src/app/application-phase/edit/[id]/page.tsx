...
import { applicationPhase, putApplicationPhase } from "@/app/service/api/applicationPhase";
import ApplicationPhaseAdapt from "@/app/service/adapt/ApplicationPhaseAdapt";

export default function EditApplicationPhase({ params }: { params: Promise<{ id: string }> }) {
    const resolvedParams = React.use(params);
    
    const formFields = new FormBuilder()
        .addTextField('fase_aplicacao', 'Fase de Aplicação *', 'text')
        .build();
    ...
    useEffect(() => {
      ...
      const getApplicationPhase = async () => {
        const dataApplicationPhase: ApplicationPhaseInterface | undefined = await applicationPhase(resolvedParams.id);
        const applicationPhaseAdapt = new ApplicationPhaseAdapt(dataApplicationPhase!);

        const applicationPhaseData = applicationPhaseAdapt.externalApplicationPhaseAdapt;

        setModel((prevModel) => {
          const updateModel = [...prevModel];

          updateModel[0].value = applicationPhaseData?.fase_aplicacao;
    ...
    const submitForm = async (e: React.FormEvent<HTMLFormElement>) => {
      ...
      const EditApplicationPhase = async () => {
        try {
          const response = await putApplicationPhase(
            { 
              id: resolvedParams.id,
              fase_aplicacao: model[0].value, 
          });
    ...
    return (
        <Base 
          title="Edição de Fase de Aplicação"
          openAlert={openAlert}
          isSuccess={isSuccess}
        ...
        >
              <div className="flex flex-col gap-6 w-full h-full z-10 relative animate-fade-up">
                  ...
                  <div className="flex flex-row w-full justify-between z-10 relative">
                      <IconButton href="/application-phase">
