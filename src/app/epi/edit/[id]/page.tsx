...
import { EmployeeLabelInterface, EpiInterface, StatusResponse } from "@/data/types";
...
import { epi, putEpi } from "@/app/service/api/epi";
import EpiAdapt from "@/app/service/adapt/EpiAdapt";
...
export default function EditProduct({ params }: { params: Promise<{ id: string }> }) {
    ...
    useEffect(() => {
      ...
      const getEpi = async () => {
        const dataEpi: EpiInterface = await epi(resolvedParams.id);
        const epiAdapt = new EpiAdapt(dataEpi);

        const epiData = epiAdapt.externalEpiAdapt;

        setModel((prevModel) => {
          const updateModel = [...prevModel];

          updateModel[0].value = String(epiData!.matricula);
          updateModel[1].value = epiData!.funcionario.id;
          updateModel[1].label = epiData!.funcionario.funcionario;
          updateModel[2].value = epiData!.conforme ? 'sim' : 'não';
          updateModel[2].label = epiData!.conforme ? 'sim' : 'não';
          updateModel[3].value = epiData!.data_abordagem;
          return updateModel;
        });
    ...
    const submitForm = async (e: React.FormEvent<HTMLFormElement>) => {
      ...
      const editEpi = async () => {
        try {
          const response = await putEpi(resolvedParams.id, parseInt(model[0].value), model[1].value, model[2].value === 'sim' ? true : false, model[3].value);
