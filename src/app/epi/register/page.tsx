...
import { postEpi } from "@/app/service/api/epi";
...
export default function RegisterEpi() {
    ...
    const formFields = new FormBuilder()
        ...
        .addTextField('data_abordagem', '* Data de Abordagem', 'date')
        .build();
    ...
    const submitForm = async (e: React.FormEvent<HTMLFormElement>) => {
      ...
      const registerEpi = async () => {
        try {
          const response = await postEpi(parseInt(model[0].value), model[1].value, Boolean(model[2].value), model[3].value);
