...
import { putTypeProduct, typeProduct } from "@/app/service/api/typeProducts";
import TypeProductAdapt from "@/app/service/adapt/TypeProductAdapt";

export default function EditTypeProduct({ params }: { params: Promise<{ id: string }> }) {
    const resolvedParams = React.use(params);
    
    const formFields = new FormBuilder()
        .addTextField('tipo', 'Tipo *', 'text')
        .build();
    ...
    useEffect(() => {
      ...
      const getTypeProduct = async () => {
        const dataTypeProduct: TypeProductInterface | undefined = await typeProduct(resolvedParams.id);
        const typeProductAdapt = new TypeProductAdapt(dataTypeProduct!);

        const typeProductData = typeProductAdapt.externalTypeProductAdapt;

        setModel((prevModel) => {
          const updateModel = [...prevModel];

          updateModel[0].value = typeProductData?.tipo;
    ...
    const submitForm = async (e: React.FormEvent<HTMLFormElement>) => {
      ...
      const EditTypeProduct = async () => {
        try {
          const response = await putTypeProduct(
            { 
