...
import { postAbsenseWarning } from "@/app/service/api/absenseWarning";

export default function RegisterAbsenceWarning() {
    ...
    const submitForm = async (e: React.FormEvent<HTMLFormElement>) => {
      ...
      const registerAbsenseWarning = async () => {
        try {
          const response = await postAbsenseWarning(parseInt(model[0].value), model[1].value, parseInt(model[2].value), parseInt(model[3].value), model[4].value);
    ...
    return (
        <Base 
          title="Cadastro de Falta/Adervertência"
          openAlert={openAlert}
          isSuccess={isSuccess}
